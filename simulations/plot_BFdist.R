...
ts.hack <- qt(ps.hack/2, df=98)
ps.nohack <- unlist(lapply(simresults.multDVhack$firstsig[cond.plot], function(x) x$ps.orig))
ts.nohack <- qt(ps.nohack/2, df=98)
library(BayesFactor)
BFs.hack <- lapply(ts.hack, function(x) ttest.tstat(x, n1=50, n2=50, simple=TRUE))
BFs.hack <- unname(simplify2array(BFs.hack))
BFs.nohack <- lapply(ts.nohack, function(x) ttest.tstat(x, n1=50, n2=50, simple=TRUE))
...
ggplot(data=newplotdat, aes(group = nDV)) +
  ...
  scale_x_continuous(breaks = log(c(1/10, 1/3, 1, 3, 10)), labels = c("1/10", "1/3", "1", "3", "10"), limits = log(c(1/11, 11)))
