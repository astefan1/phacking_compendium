...
### Hypothesis tests
We use two kinds of hypothesis tests: the independent samples t-test and univariate linear regression. The standard choice is the independent samples t-test. However, some p-hacking methods are not easily applicable to this hypothesis test or are more likely to be used in a continuous variable case. In these cases, we use a univariate linear regression.

### "Normal" versus "Ambitious" p-hacking
Most p-hacking simulation functions have an argument "ambitious". This is based on Ulrich & Miller's (2015) comment on Simonsohn et al.'s (2014) paper. They state that instead of stopping the p-hacking process as soon as they reached a significant p-value, they continue to "optimize" the p-value and choose the smallest p-value in the end. We include this strategy in the p-hacking simulation functions by selecting the smallest p-value when `ambitious = TRUE` and the first significant p-value when `ambitious = FALSE`.
...
* `ambitious`: As described above, this argument defines whether ambitious p-hacking should be applied (`ambitious = TRUE`) or not (`ambitious = FALSE`). The default setting is `TRUE`.
...
* `seed`: The initial value for the generation of random numbers in the simulation (and sometimes also p-hacking) process. The defaule is `seed = 1234`.
...
sim.multDVhack(nobs.group = 30, nvar = 5, r = 0.3, ambitious = FALSE, iter = 10, alternative = "two.sided", alpha = 0.05, seed = 1234)
```

### Selective reporting of the independent variable
We simulate this p-hacking strategy using the t-test. The **simulated dataset** contains one control group variable and multiple treatment group variables (independent variables), thus it is in a wide format. The correlation between the treatment group variables can be set using the argument `r`. The size of the groups can be defined by `nobs.group`. Note that unequal sizes of groups are not possible here because the data set is in wide format. The **p-hacking strategy** is implemented as follows: Conduct an independent samples t-test with every single treatment variable and the control group variable. For ambitious p-hacking, select smallest p-value. For normal p-hacking, select first p-value (this assumes that there is a preference relation between the treatment variables, so if the result is not significant, the non-significant p-value for the variable with the highest preference is reported).
...
sim.multIVhack(nobs.group = 30, nvar = 5, r = 0.3, ambitious = FALSE, iter = 10, alternative = "two.sided", alpha = 0.05, seed = 1234)
```

### Incorrect rounding
We simulate this p-hacking strategy using the t-test, but literally any hypothesis test should produce the same results. The **simulated dataset** contains one discrete group variable with two levels (independent variable) and one continous variable (dependent variable). The size of the groups in the p-hacking function can be defined by `nobs.group` (either an integer for equally sized groups or a vector with two elements for groups of different sizes). Note that the simulation function does not include an argument for the group size as the group size is irrelevant in this p-hacking context. The **p-hacking strategy** is implemented as follows: Compute the p-value for the t-test, if it is between the alpha level and a rounding level specified in the argument `roundinglevel` (e.g., 0.06 > p > 0.05), round it down to p = 0.05, else keep the original p-value. Note that ambitious p-hacking is not possible for this p-hacking strategy.
...
**Example**
```{r statAnalysis}
sim.impHack(nobs = 30, missing = 0.2, which = c(1:10), ambitious = TRUE, alpha = 0.05, iter = 10, seed = 1234)
```
